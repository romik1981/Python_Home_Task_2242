...
def get_jokes(count=2, can_repeat=False) -> list:
    ...
    for i in range(count):
        if can_repeat:
            words_list = list(map(lambda x: random.choice(x), [nouns, adverbs, adjectives]))
...
print(get_jokes(count=3, can_repeat=False))
print(get_jokes(can_repeat=True, count=5))
print(get_jokes())
print(get_jokes(count=7, can_repeat=False))
print(get_jokes(can_repeat=True, count=9))
